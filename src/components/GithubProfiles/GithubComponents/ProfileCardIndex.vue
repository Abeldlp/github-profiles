<template>
  <div
    ref="profileCard"
    id="card_template"
    class="flex flex-col justify-center items-center rounded-lg m-3 w-60"
  >
    <div id="profile_image" :style="imageStyle"></div>
    <h1 id="target" class="text-lg text-white m-2">
      <span class="font-bold" style="color: #2784ff">
        {{ formatName.firstLetter }}
      </span>
    ...
    <div id="target" class="flex mb-5">
      <a class="m-2" target="_blank" :href="githubProfile.html_url">
        <button
          class="py-2 px-4 rounded bg-white flex justify-center items-center"
        >
          <VueFeather type="github" class="mr-1" style="height: 15px" />
          <span>Jump</span>
        </button>
      </a>
      <DeleteButton
        ...
        @deleteProfile="removeProfileFromView"
      />
    </div>
  </div>
</template>
...
export default defineComponent({
  ...
  setup(props: Props) {
    ...
    const imageStyle = ref({
      ...
      width: "100%",
      background:
        "linear-gradient(to bottom, rgba(0, 0, 0, 0) 50%, #213C5A 100%), url(" +
        githubProfile.value.avatar_url +
        ")",
