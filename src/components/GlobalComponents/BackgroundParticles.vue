<template>
  <div class="background">
    <span
      v-for="(particle, index) in particleCount"
      :key="index"
      :style="buildRandomStyle()"
      >.</span
    >
  </div>
</template>
...
export default defineComponent({
  ...
  setup() {
    ...
    const randomNvim = (): number =>
      Math.floor(Math.random() * (1000 - 100) + 100) / 1000;
    const randomNumberTo = (num: number): number =>
      Math.floor(Math.random() * num);
    const randomizeAbsolute = (num: number): number =>
    ...
    const buildRandomOrigin = (): string => {
      return (
        randomizeAbsolute(randomNumberTo(25)) +
        "vw " +
        randomizeAbsolute(randomNumberTo(25)) +
        "vh"
      );
    ...
    const buildRandomShadow = (): string => {
      return (
        possibleNVim.value[randomNumberTo(2)] +
        " 0 " +
        randomNvim() +
        " currentColor"
      );
